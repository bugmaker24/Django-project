...
class Player extends AcGameObject {
    ...
    update_move() {
        if (this.character === "robot" && this.spent_time > 4 && Math.random() < 1 / 500.0) {
            let player = this.playground.players[Math.floor(Math.random() * this.playground.players.length)];
            let x = player.x + player.speed * this.vx * this.timedelta / 1000 * 0.3;
            let y = player.y + player.speed * this.vy + this.timedelta / 1000 * 0.3;
...
class MultiPlayerSocket {
    constructor(playground) {
        ...
        this.ws = new WebSocket("wss://app3533.acapp.acwing.com.cn/wss/multiplayer/?token=" + playground.root.access);
...
`);
    ...
    refresh_jwt_token() {
        setInterval(() => {
            $.ajax({
                url: "https://app3533.acapp.acwing.com.cn/settings/token/refresh/",
                type: "post",
                data: {
        ...
        setTimeout(() => {
            $.ajax({
                url: "https://app3533.acapp.acwing.com.cn/settings/ranklist/",
                type: "get",
                headers: {
    ...
    acwing_login() {
        $.ajax({
            url: "https://app3533.acapp.acwing.com.cn/settings/acwing/web/apply_code/",
            type: "GET",
    ...
    login_on_remote(username, password) {  // 在远程服务器上登录
        ...
        $.ajax({
            url: "https://app3533.acapp.acwing.com.cn/settings/token/",
            type: "post",
            data: {
                username: username,
                password: password,
...
getinfo_acapp() {
    let outer = this;
    $.ajax({
        url: "https://app3533.acapp.acwing.com.cn/settings/acwing/acapp/apply_code/",
        type: "GET",
        success: function(resp) {
            if(resp.result === "success") {
